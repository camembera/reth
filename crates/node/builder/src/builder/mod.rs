...
impl<Node: FullNodeTypes> BuilderContext<Node> {
    ...
    {
        self.start_network_with_policies_inner(
            builder,
            pool,
            tx_config,
            propagation_policy,
    ...
    fn start_network_with_policies_inner<Pool, N, PropPolicy, AnnPolicy>(
        &self,
        builder: NetworkBuilder<(), (), N>,
        pool: Pool,
        tx_config: TransactionsManagerConfig,
        propagation_policy: PropPolicy,
        announcement_policy: AnnPolicy,
        provenance_callback: Option<std::sync::Arc<dyn TransactionProvenanceSink>>,
    ) -> NetworkHandle<N>
    where
        N: NetworkPrimitives,
        Pool: TransactionPool<
    ...
    {
        let builder =
            builder.transactions_with_policies(pool, tx_config, propagation_policy, announcement_policy);
        let builder = if let Some(cb) = provenance_callback {
            builder.with_tx_provenance_callback(cb)
        } else {
            builder
        };

        let (handle, network, txpool, eth) =
            builder.request_handler(self.provider().clone()).split_with_handle();
